    ...
    for(int i=0; i<cdata.size(); i++) {
        ...
        while(true) {
            ...
            if(pos != -1) {
                ...
                QString n1 = rx.cap(1).replace(QRegExp("[oOîÎ]"), "0");
                QString n2 = rx.cap(2).replace(QRegExp("[oOîÎ]"), "0");
                ...
                if(resStr.contains("ïîêóïîê", Qt::CaseInsensitive) || resStr.contains("îïëàòà", Qt::CaseInsensitive)) {
                    exit = true;
                    break;
                }
                bs.append(i);
                ...
                bool adeq = true;
                while(true) {
                    int spi = resStr.indexOf(QRegExp("[ ,‚\\(\\)]"), npos);
